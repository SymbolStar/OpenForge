...
def _build_prompt(thread_id: str, agent_id: str, trigger: dict) -> str:
    ...
    return (
        ...
        f"\n"
        f"[代码改动规则 v0.5 — multi-agent workflow]\n"
        f"如果这次任务需要你修改任何 git 仓库的代码（commit / push / 开 PR），"
        f"**必须先创建 worktree，不要直接在主 repo 改**。\n"
        f"步骤：\n"
        f"  1) 判断目标仓库（看 thread 上下文 / 你自己的 STATUS）\n"
        f"  2) 跑 `openforge-worktree add {agent_id} <task-slug>`（脚本在 openforge repo 的 `bin/` 下，"
        f"已经 symlink 到 PATH；输出最后一行就是 worktree 绝对路径）\n"
        f"  3) 在这个路径下干活、commit、`git push -u origin {agent_id}/<task-slug>`、`gh pr create`\n"
        f"  4) PR 标题前缀 `[{agent_id}] `，body 里贴上当前 thread 的 link\n"
        f"  5) 任务结束（PR merge 或 abandon）后跑 `openforge-worktree rm {agent_id}/<task-slug>`\n"
        f"例外（不要建 worktree）：只读看代码 / 写文档到自己 workspace / 临时探索 / "
        f"scott 明确说「直接在主 repo 改」。\n"
        f"命名约定：branch = `{agent_id}/<task-slug>`，worktree 路径 = `<repo>.worktrees/{agent_id}-<task-slug>/`。\n"
        f"\n"
        f"[文件引用语法 v0.8]\n"
